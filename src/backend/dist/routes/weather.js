...
router.get("/weather", async (req, res) => {
    ...
    try {
        ...
        if (place) {
            ...
            try {
                const geoResp = await fetch(`https://nominatim.openstreetmap.org/search?format=json&limit=1&q=${encodeURIComponent(place)}`, { headers: { "User-Agent": "project-sistemas-d/1.0 (contact)" } });
                if (geoResp.ok) {
                    ...
                    if (Array.isArray(geoJson) && geoJson.length > 0) {
                        ...
                        console.warn("Nominatim returned no results for", place, "; falling back to q=place");
                        url.searchParams.append("q", place);
                    }
                }
                else {
                    // If geocoding service failed, fall back to sending the place as 'q' to the upstream provider.
                    console.warn("Nominatim geocoding error (status", geoResp.status, ") - falling back to q=place");
                    url.searchParams.append("q", place);
                }
            }
            catch (geoErr) {
        ...
        for (const [key, value] of Object.entries(req.query)) {
            if (["place", "latitude", "longitude"].includes(key))
                continue;
